...
function validateBMIRecordForm(bmiData){
  ...
  if(bmiData.has("userName") && bmiData.has("weight") && bmiData.has("height") && bmiData.has("age") && bmiData.has("gender")){
    ...
    let gender = String(bmiData.get("gender"));
    console.log("FUCK NOGET LORT "+gender);

    if(gender==="Male"||gender==="Female"||gender==="IDK"){
      if((minHeight <= height) && (height <= maxHeight) && (minWeight <= weight) && (weight <=maxWeight) && age<=100 && age>=5) {
          //return a fresh object with ONLY the validated fields
          let validBMIData={userName: userName, height: height, weight: weight, age: age, gender: gender};
...
`;
if(bmiStatus.gender==="Male" && bmiStatus.bmi >30){
  page+=`FAT FUCK DO NOT EAT SO MUCH`;
}
if(bmiStatus.gender==="Female"){
  page+=` GIGA SMASHHHH`;
}
page+=`</output><p><a href="/">Restart</a> | <a href="/html/help.html">Help</a> | <a href="/bmi-records?userName=${bmiStatus.userName}">See Status for User</a></p></section></body>`;
 return page;
}
...
function renderHTMLBMIStatPage(validBMIStatData){
  ...
  <p><a href="/">Restart</a> | <a href="/html/help.html">Help</a> | <a href="/bmi-records?userName=${validBMIStatData.userName}">See Status for User</a></p>
  </body>`
  return page;
}
